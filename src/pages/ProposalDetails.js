...
import { Flex, Text, Heading, ListItem, OrderedList, Link, Button, Badge } from '@chakra-ui/react';
...
export default function ProposalDetails() {
	...
	return (
		...
		<Flex m="4" flexDir="column">
		<Flex>
		<Text color="gray.400" fontWeight="bold" fontSize="xl">{moment(proposal.timestamp).fromNow()}</Text>
		</Flex>
		<Flex><Text>{proposal.text}</Text></Flex>

		<Flex m="4">
		<OrderedList>
		{proposal.links.map((link, index) => (
			<ListItem color={"#5BB0FF"} key={index}><Link href={link.url}>{link.title}</Link></ListItem>
		...
		<Flex m="2">
		<Text>Votes in favour: <Text color="green" fontWeight="bold" as="span">{proposal.upvotes}</Text> | </Text>
		<Text> {' '} &nbsp; Votes against: <Text color="brand.400" fontWeight="bold" as="span">{proposal.downvotes}</Text></Text>
		</Flex>
		<Flex>
		<Button m="2" bg="green" color="white">Upvote</Button>
		<Button m="2" bg="brand.400" color="white">Downvote</Button>
		</Flex>
		</Flex>

		<Flex>
		<Text color="gray.400" fontWeight="bold">Discussion</Text>
		</Flex>
		</Flex>
